...
PLSIM_test_split_ratio_cross2 <- function(
  ...
  calc_Zn_between_halves <- function(train_fit, test_fit) {
    ...
    linear_orthogonalize <- function(f_input) {
      ...
      beta_path <- as.matrix(fit_gam$beta)
      
      W_mat <- X_use %*% beta_path
...
result$stat_12$Zn   # -0.817071
result$stat_12$p_value   # 0.7930561

result$stat_21$Zn   # -2.188544
result$stat_21$p_value  # 0.985685

result$p_CCT   # 0.9730163
...
# 
# 
# file.exists("R_session_info.txt")
