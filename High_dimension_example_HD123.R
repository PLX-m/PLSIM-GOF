...
PLSIM_test_split_ratio_cross2 <- function(
  ...
  run_one_half <- function(id_half, seed) {
    ...
    rf_final <- randomForest::randomForest(
      ...
      y = r_rf,
      ntree = 500,
      mtry = max(5, floor(0.1*(ncol(X_rf_std) + ncol(Z_rf_std)))),
      nodesize = 5
    )
...
generate_PLSIM_data <- function(
  ...
  if (scenario == "case1") {
    ...
    dev <- 3*X[,2]^2 + sin(X[,4]+X[,5])^3 + abs(X[,6]) + cos(X[,7]+X[,8])  
    
  } else if (scenario == "case2") {
    if (q < 4) stop("case2 需要 q >= 4")
    dev <- Z[,1]^3 + Z[,2]^2 + 0.5*exp(Z[,3])^3 + 3*abs(Z[,5]) + cos(Z[,6]+Z[,7])  # t_val^2
    # dev <- Z[,1]^3 + Z[,2]^2 + exp(Z[,3])^3 + sin(Z[,4])^3 + 3*abs(Z[,5]) # cos(2*t_val)
    
  } else if (scenario == "case3") {
    if (p < 4 || q < 3) stop("case3 需要 p >= 4 且 q >= 3")
    # exp(-t_val^2)
    dev <- 2*Z[,1]^3*abs(X[,1]) + Z[,2]^2 + exp(Z[,3])^3 + sin(Z[,4]+X[,2])^3 + abs(Z[,5]) + cos(Z[,6]+Z[,7]) + (abs(X[,3]))^2
...
data <- generate_PLSIM_data(n = n, p = p, q = q, s = s, scenario = "case2", c = 1)
Y = data[[1]]
X = data[[2]]
Z = data[[3]]
...
res_size <- run_power_PLSIM(
  n_sims = 1000, n = 1000, p = 2000, q = 2000, s = 20, scenario = "H0",
  split_ratio = c(4, 4), K_si = 4, C_lambda = 0.5,
  alpha = 0.05, ncores = 10, seed = 123
)


setwd("results/")
save(res_size, file = "t_val^2_n1000_d2000_s20_nsims100_res_size.RData")
